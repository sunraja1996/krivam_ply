...
const iconMap: { [key: string]: JSX.Element } = {
  ...
  "Moisture Resistance": <BsMoisture className="w-6 h-6" />,
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 sm:gap-16 md:gap-20 items-center">
        ...
        <div className="max-w-full lg:max-w-lg text-center md:text-left">
          ...
          <p className="text-gray-700 dark:text-gray-400 mb-8">{product.description}</p>
          ...
          <div className="max-w-md w-full space-y-3 p-4 md:p-6 mb-5 rounded-lg">
            {product.features.map((feature, index) => (
              <div key={index} className="flex items-center space-x-4 p-3 rounded-lg">
                ...
                {iconMap[feature] || <ShieldCheckIcon className="w-6 h-6 sm:w-8 sm:h-8" />}
                </div>
                <h4 className="text-lg text-left font-medium text-gray-900 dark:text-white">{feature}</h4>
              </div>
            ))}
          </div>
          ...
          <div className="mb-8">
            <h3 className="text-2xl font-semibold text-gray-800 dark:text-white mb-4">Applications:</h3>
            <ul className="list-disc list-inside text-gray-700 dark:text-gray-300">
              {product.applications.map((app, index) => (
                <li key={index} className="text-lg text-left">{app}</li>
              ))}
            </ul>
          </div>
        </div>
